...
var jst_image_folder = "tree_images"
var jst_image_folder_user = "tree_images"
var jst_root_image = "accept"
	...
	if(ctlImg.id != "rootFolder"){
		...
		if(childExists(tr)){
			...
			if(s.display == ""){
				...
				fldImg.src = jst_image_folder_user + parImg + "_add.png"
			}
			jst_loaded = true
			return
		}else{
...
function renderChildren(strData, tblCls, menu){
	...
	for(var n in nodes){
		...
		code.push(' alt="" /></td><td' + (n0[1][5] ? ' background="' + n0[1][5] + '"' : '') + '><img' + (n0[1][4] ? ' class="action" onclick="showMenu(\'' + strData + '[' + n + ']\', this, event)"' : '') + ' src="' + jst_image_folder_user + n0[1][2] + '.png" alt="" /><a' + (n0[1][3] ? ' onmouseover="Tip(\'' + n0[1][3] + '\')"' : '') + ' onmouseout=UnTip() onclick="nodeClick(this)" class="tree" href=' + "'" + (menu ? String(n0[1][0]).replace(/{@strData}/g, strData) : n0[1][0] ) + "'" + (n0[1][1] ? ' target="' + n0[1][1] + '"' : jst_target ? ' target="' + jst_target + '"' : '') + '>' + n0[0]  + '</a></td></tr>')
	}
	code.push('</table>')
...
function showMenu(strData, img, event){
	...
	Tip(renderChildren(strData + "[1][4]", "menu", true),FOLLOWMOUSE,false,CLICKCLOSE,false,CLOSEBTN,true,STICKY,true,OFFSETY,-8,OFFSETX,-2,BORDERSTYLE,'none',BORDERCOLOR,'',ABOVE,false,LEFT,false,CLOSEBTNCOLORS,['','#000000','#000000','#CCCCCC'],DURATION,10000,FADEOUT,500,FADEIN,200)
